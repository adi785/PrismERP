...
const StockList: React.FC<{ store: any }> = ({ store }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!term) return store.stockItems;
    
    return store.stockItems.filter((item: StockItem) => 
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex gap-4 no-print shadow-sm">
        <div className="relative flex-1 group">
          ...
          <input 
            type="text" 
            placeholder="Search SKU by nomenclature, SKU, HSN, or unit..." 
            value={searchTerm} 
            onChange={e => setSearchTerm(e.target.value)} 
            className="w-full pl-12 pr-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 outline-none focus:ring-4 focus:ring-blue-500/10 text-sm font-bold shadow-inner placeholder:text-slate-300 dark:placeholder:text-slate-600 dark:text-white transition-all" 
          />
        </div>
      </div>
        ...
        {filteredItems.map((item: StockItem) => (
          <div key={item.id} onClick={() => setSelectedStockId(item.id)} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:border-blue-500 hover:shadow-2xl transition-all group cursor-pointer active:scale-95 duration-300">
            <div className="flex justify-between items-start mb-8">
              <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner"><Package size={28} /></div>
              <div className="flex flex-col items-end gap-2">
                 <span className={`text-[9px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest ${item.currentStock < 5 ? 'bg-rose-50 dark:bg-rose-900/20 text-rose-600' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600'}`}>{item.currentStock < 5 ? 'Low Stock' : 'Available'}</span>
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-1 group-hover:text-blue-500 transition-colors tracking-tight">{item.name}</h3>
            <div className="flex flex-wrap gap-2 mb-8">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{item.sku}</p>
              {item.hsn && <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest px-2 py-0.5 border border-slate-100 dark:border-slate-800 rounded">HSN: {item.hsn}</span>}
            </div>
            <div className="flex justify-between items-end pt-6 border-t border-slate-50 dark:border-slate-800">
              <div>
                ...
                <p className="text-2xl font-black text-slate-800 dark:text-white leading-none">{item.currentStock} {item.unit}</p>
              </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl overflow-hidden border border-white/20 animate-in zoom-in-95 duration-300">
            <div className="p-10 bg-slate-900 text-white flex justify-between items-center">
              <div><h2 className="text-3xl font-black tracking-tight">Item Registration</h2><p className="text-slate-400 mt-1 text-sm">Configure SKU master and tracking type</p></div>
