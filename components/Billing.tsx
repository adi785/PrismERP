...
import { Plus, Trash2, Printer, Save, CheckCircle2, ArrowLeft, Package, User, Calculator, Search, X, ChevronDown, Loader2, FileText, ShieldCheck, CreditCard, Building, IndianRupee, AlertCircle, Percent, Banknote } from 'lucide-react';
...
const Billing: React.FC<{ store: any, onComplete: () => void }> = ({ store, onComplete }) => {
  ...
  const totals = useMemo(() => {
    ...
    return { subtotal, discount, taxable, igst, cgst, sgst, total, totalQty, gstClasses, hasDeficit };
  }, [items]);

  const addItem = () => {
    const newItem: BillingItem = { id: Date.now().toString(), itemId: '', name: '', sku: '', hsn: '', quantity: 1, mrp: 0, rate: 0, discountType: 'percentage', discountValue: 0, discountAmount: 0, cgstRate: 9, sgstRate: 9, igstRate: 18, cgstAmount: 0, sgstAmount: 0, igstAmount: 0, amount: 0, totalWithTax: 0, availableStock: 0 };
      ...
      if (item.id === id) {
        ...
        if (updates.itemId) {
          ...
          if (stock) {
            ...
            updated.rate = stock.salePrice; updated.mrp = stock.salePrice * 1.2; 
              ...
              <div className="bg-white rounded-[3.5rem] p-12 border border-slate-100 shadow-xl shadow-slate-200/50">
                ...
                <div className="space-y-10">
                  {items.length === 0 ? (
                    ...
                    <div key={item.id} className="p-10 bg-slate-50/50 rounded-[3rem] border border-slate-100 space-y-10 relative group hover:bg-white hover:shadow-2xl transition-all duration-500" ref={activeSearchId === item.id ? searchRef : null}>
                      <div className="grid grid-cols-12 gap-6 items-end">
                        ...
                        <div className="col-span-1">
                          ...
                          <input 
                            type="number" 
                            className={`w-full px-4 py-6 rounded-2xl bg-white border-2 font-black text-base text-center outline-none focus:ring-4 transition-all ${item.itemId && item.quantity > item.availableStock ? 'border-rose-500 focus:ring-rose-500/10' : 'border-slate-100 focus:ring-blue-500/10'}`} 
                            value={item.quantity || ''} 
                        ...
                        <div className="col-span-2">
                          ...
                          <input 
                            type="number" 
                            step="0.01"
                            className="w-full px-6 py-6 rounded-2xl bg-blue-50/30 border-2 border-blue-200 font-black text-base text-right font-mono outline-none focus:ring-4 focus:ring-blue-600/20 focus:border-blue-600 transition-all shadow-inner" 
                            value={item.rate || ''} 
                        ...
                        <div className="col-span-3">
                          <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-2">Discount Adjustment</label>
                          <div className="flex bg-white border-2 border-slate-100 rounded-2xl overflow-hidden focus-within:ring-4 focus-within:ring-blue-500/10 transition-all">
                             <input 
                                type="number" 
                                className="flex-1 px-4 py-6 font-black text-base outline-none text-right placeholder:text-slate-200" 
                                placeholder="0"
                             ...
                             <button 
                                ...
                                className="px-6 bg-slate-50 border-l border-slate-100 flex items-center justify-center hover:bg-slate-100 transition-colors group/unit"
                             >
                                {item.discountType === 'percentage' ? (
                                  <Percent size={18} className="text-blue-600 group-hover/unit:scale-110 transition-transform" />
                                ) : (
                                  <Banknote size={18} className="text-emerald-600 group-hover/unit:scale-110 transition-transform" />
                                )}
                             </button>
                          </div>
                        </div>
                        <div className="col-span-1 flex items-center justify-end pb-2">
                          <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="p-4 text-rose-200 hover:text-rose-600 hover:bg-rose-50 rounded-2xl transition-all"><Trash2 size={24} /></button>
                        </div>
                      </div>
                      ...
                      {item.itemId && (
                        <div className="flex items-center justify-between px-8 py-5 bg-white rounded-3xl border-2 border-slate-100 shadow-sm group-hover:border-blue-100 transition-colors">
                           <div className="flex gap-12">
                              <div><span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1.5 block">Stock Level</span><span className={`text-sm font-black ${item.quantity > item.availableStock ? 'text-rose-600' : 'text-slate-700'}`}>{item.availableStock} Available</span></div>
                              <div><span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1.5 block">Tax Setup</span><span className="text-sm font-black text-blue-600">GST {item.igstRate}%</span></div>
                              <div><span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1.5 block">Deduction</span><span className="text-sm font-black text-rose-500">-₹{item.discountAmount.toLocaleString()}</span></div>
                              {item.quantity > item.availableStock && (
                                <div className="flex items-center gap-2 text-rose-600 animate-pulse">
                                  <AlertCircle size={14} />
                                  <span className="text-[10px] font-black uppercase tracking-widest">Inventory Shortage</span>
                                </div>
                              )}
                           </div>
            ...
            <div className="space-y-10">
              <div className="bg-slate-950 p-12 rounded-[4rem] text-white shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <h3 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] mb-14 border-b border-slate-900 pb-8">Financial Reconciliation</h3>
                  <div className="space-y-10">
                    <div className="flex justify-between items-center text-slate-400 font-black">
                      <span className="text-sm uppercase tracking-[0.2em]">Gross Value</span>
                      <span className="font-mono text-2xl text-white">₹{totals.subtotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center text-slate-400 font-black">
                      <span className="text-sm uppercase tracking-[0.2em]">Less Total Discount</span>
                      <span className="font-mono text-2xl text-rose-400">-₹{totals.discount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center text-slate-400 font-black">
                      <span className="text-sm uppercase tracking-[0.2em]">GST Total</span>
                    ...
                    <div>
                      <p className="text-[11px] font-black text-blue-600 uppercase tracking-[0.3em] mb-4">Total Amount Due</p>
              ...
              <button 
                ...
                className="w-full py-12 bg-blue-600 hover:bg-blue-700 text-white rounded-[4rem] font-black text-3xl flex items-center justify-center gap-5 transition-all shadow-[0_40px_80px_-20px_rgba(37,99,235,0.5)] group disabled:opacity-50 hover:scale-[1.01] active:scale-[0.98]"
              >
                {isSaving ? <Loader2 className="animate-spin" size={40} /> : totals.hasDeficit ? <><AlertCircle size={40} /> Insufficient Stock</> : <><ShieldCheck size={40} className="group-hover:rotate-12 transition-transform duration-500" /> Authorize & Save</>}
              </button>
            </div>
          </div>
        </div>
